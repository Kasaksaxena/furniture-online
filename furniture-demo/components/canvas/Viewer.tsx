...
export default function Viewer({ color, texture, backStyle, armStyle, zoom, rotation, showFloor }: ViewerProps) {
  const textureProps = {
          ...
          <Stage intensity={0.6} environment="city" adjustCamera={false}>
            <Float speed={1.5} rotationIntensity={0.1}>
              <group rotation={[0, rotation, 0]}>
                ...
                {/* 1. SEAT BASE (Width label removed) */}
                ...
                <group position={[0, 0.7, -0.7]}>
                  {backStyle === 'classic' && (
                    <mesh castShadow><boxGeometry args={[1.6, 1.2, 0.15]} /><meshStandardMaterial color={color} {...textureProps[texture]} /></mesh>
                  )}
                  {backStyle === 'cross' && (
                    <group>
                      <mesh rotation={[0, 0, Math.PI / 4]} castShadow><boxGeometry args={[2.2, 0.1, 0.1]} /><meshStandardMaterial color="#1e293b" /></mesh>
                      <mesh rotation={[0, 0, -Math.PI / 4]} castShadow><boxGeometry args={[2.2, 0.1, 0.1]} /><meshStandardMaterial color="#1e293b" /></mesh>
                      <mesh position={[0, 0.6, 0]} castShadow><boxGeometry args={[1.6, 0.15, 0.15]} /><meshStandardMaterial color="#1e293b" /></mesh>
                    </group>
                  )}
                  {backStyle === 'ergonomic' && (
                    <mesh rotation={[0, Math.PI / 2, 0]} castShadow>
                      <cylinderGeometry args={[0.8, 0.8, 1.4, 32, 1, true, 0, Math.PI]} />
                      <meshStandardMaterial color={color} side={2} {...textureProps[texture]} />
                    </mesh>
                  )}
                </group>
                ...
                {armStyle !== 'none' && (
                  <group position={[0, 0.35, 0]}>
                    <mesh position={[-0.85, 0, 0]} castShadow>
                      {armStyle === 'modern' ? <boxGeometry args={[0.15, 0.5, 1.2]} /> : <cylinderGeometry args={[0.08, 0.08, 1]} rotation={[Math.PI / 2, 0, 0]} />}
